...
func TestFilePerm(t *testing.T) {
	...
	if file.Mode() != 0o666 {
		t.Fatal("unexpected trace file permission")
	}
	os.Remove("test.log")
}
...
func TestFileWithPrefixPath(t *testing.T) {
	log := NewLogger(10000)
	log.SetLogger("file", `{"filename":"trace/test.log"}`)
	log.Debug("debug")
	log.Informational("info")
	log.Notice("notice")
	log.Warning("warning")
	...
	log.Critical("critical")
	log.Emergency("emergency")
	_, err := os.Stat("trace/test.log")
	if err != nil {
		t.Fatal(err)
	}
	os.Remove("trace/test.log")
	os.Remove("trace")
}
