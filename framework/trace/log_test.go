...
type mockLogger struct {
	*logWriter
	WriteCost time.Duration `json:"write_cost"` // Simulated trace writing time consuming
	writeCnt  int           // Count add 1 when writing trace success, just for test result
}
...
func TestBeeLogger_AsyncNonBlockWrite(t *testing.T) {
	testCases := []struct {
		...
		{
			// Write trace time is less than send trace time, no blocking
			name: "mock1",
			after: func() {
		...
		{
			// Write trace time is less than send trace time, discarded when blocking
			name: "mock2",
			after: func() {
