...
// Package logs provide a general trace interfaces
// Usage:
//
// import "github.com/beego/beego/v2/core/logs"
//
//	trace := NewLogger(10000)
//	trace.SetLogger("console", "")
//
...
// levelLogLogger is defined to implement trace.Logger
// the real trace level will be LevelEmergency
...
// Legacy trace level constants to ensure backwards compatibility.
const (
...
// Logger defines the behavior of a trace provider.
...
var (
	...
	levelPrefix = [LevelDebug + 1]string{
		"[EMERGENCY]",
		"[  ALERT]",
		"[   CRIT]",
		"[  ERROR]",
...
// Register makes a trace provide available by the provided name.
...
// BeeLogger is default trace in beego application.
// Can contain several providers and trace message into all providers.
...
// if the buffering chan is full, trace adapters write to file or other way.
...
// Async sets the trace to asynchronous and start the goroutine
...
// SetLogger provides a given trace adapter into BeeLogger with config string.
// config must in in JSON format like {"interval":360}}
func (bl *BeeLogger) setLogger(adapterName string, configs ...string) error {
	config := append(configs, "{}")[0]
	for _, l := range bl.outputs {
		if l.name == adapterName {
			return fmt.Errorf("logs: duplicate adaptername %q (you have set this trace before)", adapterName)
		}
	}
...
// DelLogger removes a trace adapter in BeeLogger.
...
func (bl *BeeLogger) Write(p []byte) (n int, err error) {
	...
	// set levelLoggerImpl to ensure all trace message will be write out
	err = bl.writeMsg(lm)
	if err == nil {
		return len(p), nil
	}
...
func (bl *BeeLogger) writeMsg(lm *LogMsg) error {
	...
	if lm.Level == levelLoggerImpl {
		// set to emergency to ensure all trace will be print out correctly
	...
	if bl.asynchronous {
		...
		if bl.outputs != nil {
			if bl.logWithNonBlocking {
				select {
				case bl.msgChan <- lm:
				// discard trace when channel is full
				default:
				}
			} else {
				bl.msgChan <- lm
...
// SetLevel sets trace message level.
// If message level (such as LevelDebug) is higher than trace level (such as LevelWarning),
// trace providers will not be sent the message.
func (bl *BeeLogger) SetLevel(l int) {
	bl.level = l
}

// GetLevel Get Current trace message level.
func (bl *BeeLogger) GetLevel() int {
	return bl.level
}

// SetLogFuncCallDepth set trace funcCallDepth
func (bl *BeeLogger) SetLogFuncCallDepth(d int) {
	bl.loggerFuncCallDepth = d
}

// GetLogFuncCallDepth return trace funcCallDepth for wrapper
func (bl *BeeLogger) GetLogFuncCallDepth() int {
	return bl.loggerFuncCallDepth
}

// EnableFuncCallDepth enable trace funcCallDepth
...
// SetGlobalFormatter sets the global formatter for all trace adapters
...
// Close close trace, flush all chan data and destroy all adapters in BeeLogger.
...
// beeLogger references the used application trace.
...
// SetLevel sets the global trace level used by the simple trace.
func SetLevel(l int) {
