...
type GameRoundCache struct {
	*BaseCache //基础缓存结构 封装解析与反解析函数

	Data *types.GameRoundDTO //缓存中实际存储的结构
...
*/
func (c *GameRoundCache) Get() (success bool) {
	var (
		val string
		err error
	)
	msgHeader := fmt.Sprintf("GameRoundCache traceId=%v, roomId=%v",
		c.TraceId, c.RoomId)
	...
	//解析缓存信息
	gameInfo := new(types.GameRoundDTO)
	if err = json.Unmarshal([]byte(val), gameInfo); nil != err {
		trace.Error("%v, json unmarshal failed, table=%v, field=%v, val=%v, err=%v", msgHeader, redisKey.Key,
			redisKey.Field, val, err.Error())
		return
	}
	c.Data = gameInfo
	trace.Debug("%v, userInfo=%+v", msgHeader, gameInfo)
...
执行成功返回true 失败返回false
*/
func (c *GameRoundCache) Set(srcDto *types.GameRoundDTO) (success bool) {
	var (
		val    int64
		err    error
		data   []byte
		curDto *types.GameRoundDTO
	)
	if srcDto == nil {
		curDto = c.Data
	} else {
	...
	trace.Info("%v, val=%v, GameRoundCache=%+v", msgHeader, val, c.Data)
	return true
}
...
*/
func (c *GameRoundCache) redisKey() *rediskey.RedisInfo {
	keyInfo := rediskey.GetGameRoundDetailRedisInfo(strconv.FormatInt(c.RoomId, 10), c.GameRoundId)
...
*/
func (c *GameRoundCache) redisLock() *rediskey.RedisLockInfo {
