...
// Insert 增
func (g *GoWorkerNode) Insert() int {
	o := GetUidDBOrm()

	effected, err := o.Insert(g)
	if nil != err {
		trace.Error("GoWorkerNode Insert failed, effected line=%v, error=%v, data=%+v", effected, err.Error(), g)
		return errcode.DBErrorNotOk
	}
	trace.Info("GoWorkerNode Insert success, effected line=%v, data=%+v", effected, g)

	return errcode.DBErrorOK
}

// QueryMaxId 查询Id最大的数据
func (g *GoWorkerNode) QueryMaxId() int {
	o := GetUidDBOrm()
	...
	if errors.Is(err, orm.ErrNoRows) {
		...
		trace.Error("GoWorkerNode QueryMaxId query error=%v", err.Error())
		return errcode.DBErrorNotOk
	}
	trace.Info("GoWorkerNode QueryMaxId query success, data=%+v", g)
...
// QueryCondition 使用SQL查询
func (g *GoWorkerNode) QueryCondition() {
	var (
		nodes []GoWorkerNode
		node  GoWorkerNode
	)
	...
	if nil != err {
		trace.Error("GoWorkerNode QueryCondition failed, err=%v", err.Error())
		return
	}
	trace.Info("GoWorkerNode QueryCondition success, num=%v", num)
	for index, node := range nodes {
		trace.Info("GoWorkerNode QueryCondition success, index=%v, node=%v", index, node)
	}
}
